## Inspired by: https://github.com/SY575/CMPNN
...
class MPNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int, targs):
        super(MPNEncoder, self).__init__()
        # targs
        self.r =  targs['r']
        self.task_type = targs['task_type']
        self.node_feat_size= targs['node_feat_size']
        self.dropout=targs['dropout']
        self.d_a =targs['d_a']
        self.in_channels =targs['in_channels']
        self.cnn_channels =targs['cnn_channels']
        self.cnn_layers =targs['cnn_layers']
        self.hid_dim =targs['hid_dim']
        self.n_layers =targs['n_layers']
        self.n_heads =targs['n_heads']
        self.pf_dim =targs['pf_dim']
        self.dense_hid=targs['dense_hid']
        self.n_classes=targs['n_classes']
        ...
        # tmodel
        self.DynCNN = DrugVQA(self.d_a,self.r,self.in_channels,self.cnn_channels,self.cnn_layers)
        self.decoder = Decoder(self.node_feat_size, self.hid_dim, self.n_layers, self.n_heads, self.pf_dim, DecoderLayer, 
                                SelfAttention, PositionwiseFeedforward,self.dropout)
        self.linear_final_step = torch.nn.Linear(self.hid_dim*2,targs['dense_hid'])
        self.linear_final = torch.nn.Linear(targs['dense_hid'],targs['n_classes'])
        

    def forward(self,mol_graph: BatchMolGraph, features_batch=None, contactmap=None) -> torch.FloatTensor:

        #Dyncnn
        protein_feats,seq_att = self.DynCNN(contactmap.unsqueeze(0))
        ...
        for depth in range(self.depth - 1):
            ...
            #intercattention
            protein_feats,message_atom=self.decoder(protein_feats,message_atom.unsqueeze(0))
            message_atom = message_atom.squeeze(0)
        ...
        #intercattention
        protein_feats,atom_hiddens=self.decoder(protein_feats,atom_hiddens.unsqueeze(0))
        atom_hiddens = atom_hiddens.squeeze(0)
        ...
        #fc
        if not bool(self.task_type):
            return F.sigmoid(self.linear_final(F.relu(self.linear_final_step(torch.cat([(torch.sum(protein_feats,1)/self.r).unsqueeze(0),mol_vecs.unsqueeze(0)],dim=2))))),seq_att
        else:
            return F.log_softmax(self.linear_final(F.relu(self.linear_final_step(torch.cat([(torch.sum(protein_feats,1)/self.r).unsqueeze(0),mol_vecs.unsqueeze(0)],dim=2))))),seq_att
...
class MPN(nn.Module):
    def __init__(self,
        ...
        self.encoder = MPNEncoder(self.args, self.atom_fdim, self.bond_fdim,targs)
        self.task_type = targs['task_type']
